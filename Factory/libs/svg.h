...
inline Element* Element::get_element_by_id(const std::string& id)
{
	...
	for (auto& current : child_elems)
		if (current->attr.find("id") != current->attr.end() && current->attr.find("id")->second == id)
			return current;
...
public:
	...
	double rx() { return this->find_numeric("rx"); }
	double ry() { return this->find_numeric("ry"); }
	virtual double x() override { return this->find_numeric("cx"); }
	virtual double y() override { return this->find_numeric("cy"); }
	virtual double width() override { return this->rx() * 2; }
	virtual double height() override { return this->ry() * 2; }
	Element::BoundingBox get_bbox() override;

protected:
	std::string tag() override { return "ellipse"; }
};
...
{
	double x = this->x(), y = this->y(), rx = this->rx(), ry = this->ry();
...
inline std::string SVG::Style::svg_to_string(const size_t indent_level)
{
	...
	if (!this->css.empty() || !this->keyframes.empty())
	{
		...
		{
			ret += indent + "\t\t@keyframes " + anim.first + " {\n" + to_string(anim.second, indent_level + 1) + indent + "\t\t" + "}\n";
		}
